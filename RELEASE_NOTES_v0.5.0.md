...
yours to set. Based on the dependency shape already visible in ADR-0002's
responsibility list: lower-dependency, higher-reuse subsystems first,
`agents`/`workflow` last since they're consumers of everything else.
...
| 8 | **First concrete provider implementation** | Directly addresses the "unproven contract" limitation above — proves `BaseProvider` against something real before `agents` is built on top of it. Does not require a new subsystem package, just a first implementation inside `providers`. |
